...
export default function ApplicationExecutorsContainer({
  ...
  return (
    <div className="bg-[#FEFEFE] flex flex-col gap-3 p-6">
      ...
      {note ? (
        <div className="grid grid-cols-[minmax(0,1fr)_228px_minmax(0,1fr)] gap-x-5 gap-y-3 border-t border-[#F5F5F5] px-4 pt-4">
          {changedByName ? (
            <p className="text-sm leading-5 text-[#1F1F1F]">
              {changedByName}
            </p>
          ) : null}
          {createdAt ? (
            <p className="text-sm leading-5 text-[#797979]">{createdAt}</p>
          ) : null}
          <p className="whitespace-pre-wrap text-sm leading-5 text-[#1F1F1F]">
            Qeyd:  {note}
          </p>
        </div>
      ) : null}
