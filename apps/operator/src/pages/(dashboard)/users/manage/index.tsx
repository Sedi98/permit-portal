...
function UserEditForm({
  ...
  const [fin, setFin] = React.useState(user.fin);
  const [role, setRole] = React.useState<Exclude<Role, "super_admin">>(
    user.role === "super_admin" ? "executor" : user.role,
  );
    ...
    update.mutate(
      {
        ...
        fin,
        role,
        department_id: requiresDepartment ? Number(departmentId) : null,
        is_active: isSelf ? user.is_active : isActive,
      },
      {
        onSuccess: () => toast.success("İstifadəçi yeniləndi"),
        onError: () => toast.error("İstifadəçi yenilənərkən xəta baş verdi"),
      },
    );
  };
  ...
  const deactivate = () => {
    if (isSelf) {
      toast.error("Öz hesabınızı deaktiv edə bilməzsiniz");
      return;
    }
    remove.mutate(user.id, {
      ...
      onError: () => toast.error("İstifadəçi deaktiv edilərkən xəta baş verdi"),
    });
  };

  return (
    <TableLayout className="max-w-2xl space-y-5">
      <h1 className="text-xl font-bold">İstifadəçini redaktə et</h1>
      <div className="space-y-2">
        <Label htmlFor="user-name">Ad</Label>
        <Input id="user-name" value={name} onChange={(event) => setName(event.target.value)} />
      </div>
      <div className="space-y-2">
        <Label htmlFor="user-fin">FİN</Label>
        <Input id="user-fin" maxLength={7} value={fin} onChange={(event) => setFin(event.target.value)} />
      </div>
      <div className="space-y-2">
        <Label>Rol</Label>
        <Select
          value={role}
          onValueChange={(value) => setRole(value as Exclude<Role, "super_admin">)}
          disabled={isSelf}
        >
      ...
      <div className="flex justify-between gap-3">
        ...
        <Button onClick={save} disabled={update.isPending}>Yadda saxla</Button>
      </div>
    </TableLayout>
  );
}
...
export default function UserManagePage() {
  ...
  return (
    <main className="space-y-5 p-4">
      <Button variant="ghost" className="gap-2 text-[#286AA6]" onClick={() => navigate(-1)}>
